...
if __name__ == '__main__':
    ...
    for (dirpath,dirnames,filenames) in os.walk(path):
        for dirname in sorted(dirnames):
            ...
            for(direcpath,direcnames,files) in os.walk(path+"/"+dirname):
                ...
                for file in file_list[:BATCH_SIZE]:
                    actual_path=path+"/"+dirname+"/"+file
                    frame = cv2.imread(actual_path)
                    des = func(frame)
                    if des is None:  # Eliminates empty items.
                        continue
    ...
    clf_knn = predict_knn(X_train, X_test,y_train, y_test)
    clf_mlp = predict_mlp(X_train, X_test,y_train, y_test)
    clf_svm = predict_svm(X_train, X_test,y_train, y_test)

    clf_lr = predict_lr(X_train, X_test,y_train, y_test)
    clf_nb = predict_nb(X_train, X_test,y_train, y_test)

    models = {
        'knn': clf_knn, 'mlp': clf_mlp, 'svm': clf_svm,
        'lr': clf_lr, 'nb': clf_nb
        }

    with open('classifiers.model', 'wb') as models_file:
